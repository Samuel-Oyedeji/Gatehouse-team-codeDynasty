...
function sortCharges(charges: ReconCharge[], rule: AllocationRule): ReconCharge[] {
  ...
  if (rule === "DUES_FIRST") {
    ...
    return [...charges].sort(
      (a, b) => rank(a) - rank(b) || byOldest(a, b),
    );
  }
...
export function reconcile(input: ReconInput): ReconResult {
  ...
  if (!input.forceApply) {
    ...
    if (isDuplicate(input)) {
      return {
        ...
        suggestion: "Same amount from the same sender within the duplicate window — hold for confirmation.",
      };
    }
  ...
  const totalOutstandingBefore = ordered.reduce((a, c) => a + c.outstandingKobo, 0);
  ...
  const outstandingAfter = totalOutstandingBefore - creditAppliedKobo -
