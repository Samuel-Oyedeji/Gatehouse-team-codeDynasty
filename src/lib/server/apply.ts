...
import type { ReconResult } from "../reconciliation";
import { deriveUnitStatus, chargeStatusFor } from "./domain-helpers";
 ...
 *  credit entries), guaranteeing balance == Σ outstanding (PRD §8). */
...
/** Immediately apply a unit's existing credit balance to its open charges,
 *  oldest-first (PRD §5.2 — billing applies existing credit at once). */
export async function consumeCreditForUnit(
  ...
  for (const c of charges) {
    ...
    await tx.creditEntry.create({
      data: { unitId, amountKobo: -take, reason: "Credit applied to new charge" },
    });
  }
...
export async function applyReconEffects(
  ...
  for (const alloc of result.allocations) {
    ...
    if (alloc.amountKobo > 0) {
      await tx.allocation.create({
        data: { paymentId, chargeId: charge.id, amountKobo: alloc.amountKobo },
      });
    }
    await tx.charge.update({
      where: { id: charge.id },
  ...
  if (result.creditAppliedKobo > 0) {
    await tx.creditEntry.create({
      data: { unitId, amountKobo: -result.creditAppliedKobo, reason: "Applied to charges" },
    });
  }
  if (result.creditAddedKobo > 0) {
    await tx.creditEntry.create({
      data: { unitId, amountKobo: result.creditAddedKobo, reason: "Overpayment surplus held as credit" },
    });
  }
